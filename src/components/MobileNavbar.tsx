...
const MobileNavbar = () => {
  ...
  const navLinks = [
    ...
    { name: "Projects", href: "/projects", icon: <FaProjectDiagram size={20} /> },
  ...
  return (
    <>
      ...
      <div className="md:hidden fixed top-4 left-4 z-50">
        <button onClick={toggleDrawer} className="text-white p-2 rounded-full">
      ...
      >
        <div className="absolute top-4 right-4">
          <button onClick={closeDrawer} className="text-white p-2 rounded-full">
            <X size={24} />
          </button>
        </div>
        ...
        {navLinks.map(({ name, href, icon }) => (
            <a
              ...
              href={href}
              onClick={closeDrawer}
              className="flex items-center justify-center gap-3 text-lg font-medium hover:text-blue-400 transition"
            >
              {icon}
              {name}
            </a>
